...
一个面向中文读者的多信源新闻追踪与来源审查工具。它从参考消息、新华社、环球网、NRK、VG 等公开媒体来源抓取新闻标题、发布时间和原文链接，筛选“中国国内时政 / 国际时政 / AI 产业”和“挪威 / NATO / EØS”相关内容，并生成一份可以直接点击跳转原文的本地网页日报。
...
- 每个来源记录抓取状态、抓取数量、当天数量、入选数量；
- 来源失败会在网页和 `data/audit.json` 里标红；
- RSS 源会检查文章日期是否等于目标日期；
...
- 增加趋势图和周报；
- 把 LLM judge 的输出结构化，减少“看起来合理但无法核查”的摘要；
- 做一个可交互网页，让用户按来源、主题、日期筛选新闻；
- 部署到 GitHub Pages 或 Hugging Face Spaces，形成可以公开访问的 demo。
